...
int main (int argc, char** argv){
    ros::init (argc, argv, "ros_opencl_int_float_vectors_test1");
    ros::NodeHandle nh;
    string kernel_filename;
    string image_topic;
    string result_topic;

    nh.param("ros_opencl_int_float_vectors_test1/kernel_filename", kernel_filename, string("test_kernels.cl"));
    nh.param("ros_opencl_int_float_vectors_test1/image_topic", image_topic, string("/usb_cam/image_raw"));
    nh.param("ros_opencl_int_float_vectors_test1/result_topic", result_topic, string("ros_opencl_int_float_vectors_test1/result"));

    string full_kernel_path = ros::package::getPath("ros_opencl") + "/tests/kernels/" + kernel_filename;

    roscl = new ros_opencl::ROS_OpenCL(full_kernel_path, "frameDiffIntFloat");
